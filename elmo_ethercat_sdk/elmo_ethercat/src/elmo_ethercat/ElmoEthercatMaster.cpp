...
bool ElmoEthercatMaster::loadSetup(const std::string& setupFile) {
  ...
  if (yamlNode["elmodrives"].IsDefined()) {
    ...
    std::cout<<"Has found "<<elmoSetup.size()<<" slaves!"<<std::endl;
    ...
    for (size_t i = 0; i < elmoSetup.size(); i++) {
      ...
      if (elmoSetup[i]["ethercat_address"].IsDefined()) {
        ...
        elmo::ethercat::ElmoEthercatSlavePtr slavePtr = std::make_shared<ElmoEthercatSlave>(elmoSetup[i]["name"].as<std::string>(), getBus(), address);
        bus_->addSlave(slavePtr);
        elmodrivePtr->setSlavePointer(slavePtr);
        elmodrivePtr->name_ = elmodrivePtr->getNameOfSlave();
        std::cout<<elmodrivePtr->getNameOfSlave()<<std::endl;
    ...
    for (size_t i = 0; i < elmoSetup.size(); i++) {
      ...
      modeofOperation_ = elmodrives_[i]->modeofOperation_;
    }
    setAllSlavesInit();
...
bool ElmoEthercatMaster::elmodriveExists(const std::string& name) const {
  for (const auto& elmodrive : elmodrives_) {
    if (elmodrive->getNameOfSlave() == name) {
      return true;
    }
  }
  return false;
...
bool ElmoEthercatMaster::addElmodrive(const ElmoEthercatPtr& elmodrivePtr) {
  const std::string name = elmodrivePtr->getNameOfSlave();
