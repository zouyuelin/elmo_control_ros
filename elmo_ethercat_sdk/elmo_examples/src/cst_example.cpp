...
bool updateController(const any_worker::WorkerEvent& event) {
  ...
  command.setModeOfOperation(ModeOfOperationEnum::CyclicSynchronousVelocityMode); //CyclicSynchronousVelocityMode CyclicSynchronousTorqueMode CyclicSynchronousPositionMode
  ...
  if (master->allDrivesChangedStateSuccessfully()) {
    ...
    if (!master->allDrivesAreInTheState(DriveState::OperationEnabled)) {
      ...
      // std::cout<<targetTorque<<std::endl;
      // command.setTargetTorque(1.5);
      if(position>200) command.setTargetVelocity(-5.0);
      else command.setTargetVelocity(0.0);
      // command.setTargetPosition(522);
      for (const auto& elmodrive : elmodrives) {
